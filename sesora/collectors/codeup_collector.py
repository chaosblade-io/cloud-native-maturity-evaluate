...
class CodeupCollector(CollectorBase):
    ...
    def _collect(self) -> List:
        ...
        # 检查 Token 是否配置
        if not self.token:
            logger.error("错误: 未配置云效 Token，请设置 YUNXIAO_TOKEN 环境变量")
            return []
        ...
        if not org_ids:
            logger.warning("警告: 未配置组织 ID，请设置 codeup_org_id 或 CODEUP_ORG_ID")
            return []

        # 为每个组织采集代码仓库
        for org_id in org_ids:
            logger.info(f"正在采集组织 {org_id} 的代码仓库信息...")
            repos = self._collect_repos(org_id, self.project)
            records.extend(repos)

            for repo in repos:
                logger.info(f"正在采集仓库 {repo.repo_name} 的信息...")
                file_tree = self._collect_repo_file_tree(
                    org_id, repo.repo_id, repo.repo_name
                )
                records.extend(file_tree)

                logger.info(f"正在采集仓库 {repo.repo_name} 的文件提交信息...")
                file_commits = []
                ...
                logger.info(f"正在采集仓库 {repo.repo_name} 的标签信息...")
                tags = self._collect_repo_tags(org_id, repo.repo_id)
                records.extend(tags)
                logger.info(f"采集到仓库 {repo.repo_name} 的 {len(tags)} 个标签")

                logger.info(f"正在采集仓库 {repo.repo_name} 的分支信息...")
                branches = self._collect_repo_branches(org_id, repo.repo_id)
                records.extend(branches)
                logger.info(f"采集到仓库 {repo.repo_name} 的 {len(branches)} 个分支")

                logger.info(f"正在采集仓库 {repo.repo_name} 的提交信息...")
            ...
            logger.info(f"正在采集组织 {org_id} 的流水线信息...")
            pipelines = self._collect_pipelines(org_id)
            records.extend(pipelines)
            logger.info(f"采集到 {len(pipelines)} 条流水线")
            ...
            for pipeline in pipelines:
                ...
                logger.info(f"采集到流水线 {pipeline.name} 的配置信息")
    ...
    def _is_pipeline_in_project(self, org_id: str, pipeline_id: str) -> bool:
        ...
        body = response.body
        if response.status_code != 200 or not body.success:
            logger.warning(f"获取流水线详情失败 (pipeline_id={pipeline_id})")
            return False
